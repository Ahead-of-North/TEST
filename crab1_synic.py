...
def ElectronSynIC(pars, data):
    ...
    e_break = (10**pars[1])*u.eV
    alpha_1 = pars[2]
    alpha_2 = pars[3]
    e_cutoff = (10**pars[4])*u.eV
    B = pars[5]*u.uG
...
def lnprior(pars):
    ...
    logprob = (
        naima.uniform_prior(10**pars[0], 0.0, np.inf)
        + naima.uniform_prior(10**pars[1], np.log10(0.1), np.log10(10))
        + naima.uniform_prior(pars[2], 0, 5)
        + naima.uniform_prior(pars[3], 0, 5)
        + naima.uniform_prior(10**pars[4], np.log10(100), np.log10(3000))
